...
async def buy(product_id: int, deal: BuyProduct, user: User = Depends(get_current_user), session: Session = Depends(get_db)):
    return await ProductManager.buy(product_id=product_id, amount=deal.amount, user=user, session=session)
