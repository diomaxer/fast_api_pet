...
@router.get('/', response_model = List[Product] | None)
...
async def create_product(product: ProductBase, user: User = Depends(get_current_user)):
    return await ProductManager.create(product=product, user=user)


@router.put('/{product_id}', response_model = Product | None)
